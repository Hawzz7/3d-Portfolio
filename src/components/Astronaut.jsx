...
// change the file name from Model to Astronaut
export function Astronaut(props) {
  ...
  //   some change here below
  useEffect(() => {
    if (animations.length > 0) {
      actions[animations[0].name]?.play();
    }
  }, [actions, animations]);
  //Som changes here below
  const yPosition = useMotionValue(5); // ✅ This value provides the initial position of the 3d model in yPosition
  const ySpring = useSpring(yPosition, { damping: 30 });// ✅ Applies a spring animation to yPosition with a damping effect (smooth deceleration)
  useEffect(() => {
    ySpring.set(-1);
  }, [ySpring]); //After the initial render this useEffect will run once and set the ySpring value from 5 to -1
  useFrame(() => {
    group.current.position.y = ySpring.get();
  }); //✅ On every frame, updates the 3D model's Y position using the current spring value (from 5 to -1) 
  return (
    <group
      ref={group}
      {...props}
      dispose={null}
      // put the rotation and scale attribute here below that you copied from  "<group name=sketchfab_model>"
      rotation={[-Math.PI / 2, -0.2, 2.2]}
      scale={props.scale || 0.3}
      //Add the positiom attribute
      position={props.position || [1.3, -1, 0]}
    >
      <group name="Sketchfab_Scene">
        <group
          name="Sketchfab_model"
          //select and cut copy the rotation and scale attribute
        >
          <group name="Root">
            <group name="metarig">
